# Sir Fish — Demo v4.5 Implementation Specification

**Document type:** build prompt for an implementing model
**Project:** `C:\Projects\Godot\Sir Fish` (Godot 4.7-stable, Forward+)
**Supersedes:** `Sir Fish - Demo v4 Implementation Spec.md` **completely**, and through it v3 and v3.5. **All are dead.** Do not read them, do not cite them, do not resolve a conflict in their favour. v1 and v2 were already dead and stay dead. **This document is the whole specification.**
**Answers:** every open item in `QUESTIONS-m8b.md`, plus everything v4 already answered from `QUESTIONS-v3.5.md` and `QUESTIONS-v3-m8a.md`. See §0.2. All questions files are **closed**.
**Source:** v4 in full, plus the partial M8b implementation pass and its questions file.

---

## 0. How to use this document

**M0 through M8b are built, gated, and verified.** §9.0.2's skeleton-pose blocker is fixed and re-verified against all three heroes. Roughly 120 source files exist. A run plays from encounter 0 to the run summary, eight headless tests pass, the upgrade loop closes, the parallax is provably seamless, the shop pauses the game, and Sir Fish is a rigged, seven-clip Blender model in a procedural tank.
...
3. **M8c, M8d, M8e** next, in order, each independently gated.
4. **The verification backlog in §24**, seven items owed from earlier gates.
...
Thirteen open items came out of `QUESTIONS-v3.5.md`, `QUESTIONS-v3-m8a.md` and `QUESTIONS-m8b.md`. All thirteen are resolved here and all three files are closed. Resolutions are labelled **R1–R13** (the `A`, `C` and `E` labels are still in use in §21 and are not reused).

**R10–R13 are new in v4.5** and are the reason this document exists; R1–R9 carry over from v4 unchanged.
...
### 0.4 What v4.5 changes

v4.5 adds **no features**. Everything v4 specified still stands, word for word, except where listed below. The changes are concentrated almost entirely in one place — how a Blender-skinned character's animation is authored — because that is where M8b ran aground.

1. **§9.0.2 is new and is the heart of this revision.** It specifies how animation deltas map onto skeleton bone tracks, why that mapping is *not* the identity, and why the placeholder rig never revealed the difference. This is the fix for the M8b blocker.
2. **§8.2b is new.** It specifies the swap architecture — what happens to `CombatantRig` and `CombatantAnimations` when a real model lands — which v4 gestured at in §4.1 and §23.3 but never stated in one place. R10 ratifies the reading the implementer arrived at.
3. **Four more questions answered** (R10–R13), one of which was a hard **BLOCKER**.
4. **Three new traps and two new accepted deviations** (§21.3 C11–C12, §21.4), including the skeleton-track trap, which is the most expensive one in the document.
5. **v4 §9.0's wording is corrected.** The sentence *"Rotations are absolute, because every home rotation is zero"* was true of the placeholder rig and is false of a skinned armature. That single unexamined clause is what made the blocker possible, and it is now scoped explicitly.
6. **Milestone status is current:** M8b is complete and gated — warrior, ranger, and priest all landed, §9.0.2's fix verified on all three.
...
static func _bone_pos_key(skel: Skeleton3D, bone_idx: int, delta: Vector3) -> Vector3:
    return skel.get_bone_rest(bone_idx).origin + delta
```
...
##### Verifying the fix, before building two more heroes

`Skeleton3D.get_bone_rest()` / `get_bone_pose()` are not exposed by any MCP tool and this build has no `execute_game_script` (§0.1.4, R13). **Add a permanent `Debug` verb rather than a throwaway** — this will be wanted again for the ranger, the priest, and both orcs:
...
Then confirm, on the warrior, in this order:

1. **At `idle` t=0 with every authored delta set to 0**, `pose` must equal `rest` for every bone. If it does not, the composition is wrong and nothing downstream is trustworthy. **This is the assertion that would have caught the bug on day one.**
2. At a known non-zero key — the attack windup's −110° — the arm reads as "sword back over the shoulder", matching the Blender viewport capture already taken this session.
3. `capture_frames` across all six clips, per §20.4's gate. A still cannot show a swing.

Only then start the ranger.
...
| `bone <combatant_id> <BoneName>` | **[v4.5]** Log that bone's `rest`, `pose` and global-pose transforms. Exists because no MCP tool exposes `Skeleton3D.get_bone_rest`/`get_bone_pose` and this build has no `execute_game_script`. **This is how §9.0.2's fix is verified**, and it is needed again for every rigged character in M8b and M8c — build it permanent, not throwaway. |
...
| **M8b** | **The three heroes** | **Complete and gated** |
...
| Material reassignment | **Done and confirmed on screen** — blue armour, red accent, gold trim per §6.1. Found and fixed a second, separate defect during gate closeout: the four `StandardMaterial3D` sub-resources hand-copied into `warrior.tscn` (§21.3's C2-style workaround) all carried an identical flat `Color(0.906, 0.906, 0.906)` instead of their correct per-part colours, so the warrior rendered as a white silhouette structurally passing the cel-shader check while visually wrong. `warrior.glb` itself always had the correct baked colours (`Warrior_Armor` 0.29/0.44/0.65, `Warrior_Accent` 0.85/0.20/0.25, `Weapon_Gold` 0.95/0.76/0.19, `Weapon_Iron` 0.55/0.58/0.64) — only the hand-copied `.tscn` text was wrong. Corrected in place; verified on screen and via `get_game_node_properties`. |
...
| Material reassignment | **Done and confirmed** — green leather / brown accent (§6.1). A material idempotency bug (second `setup()` call overwriting the correct albedo with white) was found and fixed here first, then confirmed not to have regressed. |
...
### 20.4 M8b — The three heroes *(complete and gated)*
...
Everything from v3's E1–E14 and v3.5's twelve items is **implemented and verified on disk**. v4.5 adds four edits, all in M8b's surface area:
...
| **E15** | `combatant_skeleton_animations.gd` | **Compose every bone key with `get_bone_rest()` per §9.0.2.** Rotate about the parent-space image of world +Z. Never key a bone position to `Vector3.ZERO`. | **Outstanding — this is the blocker fix.** The authored numbers in the file are correct and stay; **only the two track helpers `_rot_track` and `_pos_track` change.** No signature change is required: `SKELETON_PATH` is already relative to the player's `root_node` (`Visual`), so the live `Skeleton3D` resolves from the path the file already holds. |
| **E16** | `debug.gd` | Add the `bone <combatant_id> <BoneName>` verb (§19.2) | **Outstanding.** Build it *before* attempting E15's verification — it is the only way to see rest vs pose in this build. |
| **E17** | `combatant_rig.gd` | Correct the stale "M6 swaps meshes while keeping the animation tracks pointed at these same node paths" docstring (§8.2b) | **Outstanding.** Actively misleading now: swapped characters' tracks point at bones, not those node paths. |
...
- **The clips themselves land in Godot as GDScript-authored bone tracks, not as imported glTF channels** (C11) — this build has no tool to extract a baked `AnimationLibrary` from a `.glb`. Author them in Blender anyway: the `.blend` stays the reference for the numbers and the screen-plane check above, and if a future MCP build gains `execute_editor_script` the baked actions can be brought across verbatim. **Transcribing those numbers into bone tracks requires §9.0.2's rest composition** — the values are not portable as-is.
...
### 24.0 The one live blocker

**The M8b skeleton-pose defect is the only thing in this document that stops work.** It is diagnosed in §9.0.2 with a fix and a verification procedure; it is not an open question and needs no owner input. It is called out separately from the table below because the table's items are all "seen working" gaps, whereas this one is a broken behaviour.

**Do not build the ranger or the priest until the warrior's six clips read correctly.** M8b's per-hero gate exists precisely so a pipeline defect is found once rather than three times, and it worked — halting there was the right call.

### 24.1 Verification owed

Seven items. **None is a blocker** — every one is a screenshot or a command against behaviour that is already implemented and code-reviewed. All seven must be closed before M8e signs off, and the milestone that most naturally closes each is named.
...
**None.** Every entry in `QUESTIONS.md` (Q0–Q24), `QUESTIONS-v2.md` (V0–V15), `QUESTIONS-v3.md`, `QUESTIONS-v3.5.md`, `QUESTIONS-v3-m8a.md` and `QUESTIONS-m8b.md` is answered — the last three by §0.2's R1–R13. All six files are **closed**.

One item is a live **defect** rather than a question: the M8b skeleton-pose blocker, diagnosed and fixed in §9.0.2 and tracked in §24.0. It needs no design input.

Open a fresh `QUESTIONS-v4.5.md` rather than appending to any of them, and follow the same discipline: **implement as specified, record every deviation with the file and the reasoning, change nothing silently, and raise a BLOCKER rather than guessing** when a design decision is genuinely missing.

**Raising that blocker was correct and is worth saying so.** The M8b pass stopped one hero in, wrote down exactly what worked and what did not, listed what had been ruled out, and proposed a next diagnostic step — rather than scaling every authored angle down until the warrior looked plausible. Had it done that, the ranger and priest would have inherited the same masked defect and the real cause would have surfaced three characters later, in someone else's session. **Stopping at the gate is the gate working.**
